...
int sock_init(void)
{
  ...
  while(1){
    ...
    myaddr.sin_addr.s_addr = INADDR_ANY;//通信对象的IP地址，0.0.0.0表示所有IP地址
  ...
  while(1){
    ...
    if(ret == -1){
      ...
      return;
		...
		memcpy(msg.writer,recv_buffer+POSITION_NAME,4);
		memcpy(msg.time,recv_buffer+POSITION_TIME,26);
		memcpy(msg.content,recv_buffer+POSITION_CONTENT,128);
        printf("%s:%s\n%s",msg.writer,msg.content,msg.time);
	}
  }
}
