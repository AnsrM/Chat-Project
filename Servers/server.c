...
{
  // 打开mysqlite.db的数据库，如果数据库不存在，则创建并打开
  sqlite3_open_v2("mysqlite.db",&db,
  ...
  //ID 昵称 密码，ID为主键
  char *errmsg = NULL;
  char *sql = "CREATE TABLE if not exists user_info(id text primary key,\
...
int check_recv_id(int fd )
{
  ...
  int ret;
  int i;
  client_info client_ID_info; //帐号信息结构体ID_info
  ...
  if(recv_ID_info[0] == '1'){
	sprintf(sql,"select * from user_info where id = '%s' and passwd = '%s';",
  ...
  //注册，根据昵称和密码注册、记录帐号信息，并返回帐号
    ...
    int j = 100;
	char *sql1 = "select * from user_info;";
	...
	sprintf(sql,"insert into user_info values('%s','%s','%s'); ",client_ID_info.id,
